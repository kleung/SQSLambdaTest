...
public class TestDataSNSEventConsumer implements Function<Flux<SNSRecord>, Mono<String>> {
	...
	public TestDataSNSEventConsumer() {
		super();
	}
	...
	@Override
	public Mono<String> apply(Flux<SNSRecord> messages) {
		//throw new RuntimeException("Test failing!");
		messages.subscribe(
				(message) -> {
					//throw new RuntimeException("Test failing!");
					TestData record = this.readData(message.getSNS().getMessage());
					//throw new RuntimeException("Test failing!");
